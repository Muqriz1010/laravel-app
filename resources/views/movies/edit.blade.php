...
@section('content')
    ...
    <form method="POST" action="{{ route('movies.update', $movie->id) }}" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <input type="date" class="form-control" id="year_of_release" name="year_of_release"
                value="{{ $movie->year_of_release }}" required>
        </div>

        <div class="form-group">
            <label for="plot">Plot:</label>
            <input type="text" class="form-control" id="plot" name="plot" value="{{ $movie->plot }}" required>
        </div>

        <div class="form-group">
            <label for="image">Image:</label>
            <input type="file" class="form-control" id="image" name="image"
                src="{{ asset('images/' . $movie->image) }}" value="{{ $movie->image }}" required>
        </div>
            ...
            <div class="form-group" id="producer_list_wrapper">
                <select class="form-control" id="producer_list" name="producer_list">
                    <div class="form-group" id="options-list">
                        <option value="{{ $movie->producer->name }}"></option>
                    </div>
                </select>
            </div>
        </div>
        ...
        var ajax1 = $(document).ready(function() {
            $.ajax({
                ...
                success: function(data) {
                    ...
                    var s = '<option value="{!! $movie->producer->id !!}">{!! $movie->producer->name !!}</option>';
                    for (var i = 0; i < data.length; i++) {
                        if (data[i].id != {!! $movie->producer->id !!}) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        actorList += '<option value="' + data[i].id + '">' + data[i].name + '</option>';
                        s += '<option value="' + data[i].id + '">' + data[i].name + '</option>';
                    }
                    $('#actor_list').html(s);
                }
            ...
            $.ajax({
                type: "GET",
                url: "/movies/actors/{!! $movie->id !!}",
                data: "{}",
                success: function(data) {
                    var data = data.actors;
                    var selectOption;
                    for (var i = 0; i < data.length; i++) {
                        var tempActorList = actorList;
                        selectOption = '<option value="' + data[i].id + '">' + data[i].name +
                            '</option>';
                        tempActorList.replace(selectOption, "");
                        selectOption += tempActorList;
                        $('#dynamic_field').append('<tr id="row' + i +
                            '" class="dynamic-added"><td><select name="actor_list[]" class="form-control name_list" required id="name-two">' +
                            selectOption +
                            '</select></td><td><button type="button" name="remove" id="' + i +
